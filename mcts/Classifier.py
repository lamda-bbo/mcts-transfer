...
from mcts.utils import minmax, standardization
import os
# from bbo.algorithms.np.transformer import transformer_opt
# from turbo_1.turbo_1 import Turbo1

# the input will be samples!
class Classifier():
    def __init__(self, samples, dims, kernel_type, node = None, gamma_type = "auto", stage = 0, mode=None):
        ...
        self.gpr     =   GaussianProcessRegressor(kernel=m52, alpha=noise**2, normalize_y=False) #default to CPU
        self.algo    =   None
    ...
    def train_gpr(self, samples, lb, ub):
        X  = []
        fX  = []
    ...
    def train_gpr_local(self, samples, lb, ub):
        X  = self.X.reshape(-1, self.dims)
        fX = self.fX.reshape(-1)
        X = minmax(X, min=lb, max=ub)
        fX = standardization(fX)
        self.gpr.fit(X, fX)
    ...
    def expected_improvement(self, X, lb, ub, xi=0.0, use_ei = True):
        ...
        if not use_ei:
            ...
            mu_sample = standardization(self.fX.reshape((-1, 1)))
    ...
    def propose_rand_samples_sobol(self, nums_samples, path, lb, ub, source_samples):
        ...
        if ratio_check == 0 or len(centers) == 0:
            if source_samples and len(source_samples) > 0:
                centers = [sample[0] for sample in source_samples[:10]]
                # centers = [source_samples[0][0]]
            else:
                return self.propose_rand_samples( nums_samples, lb, ub )
        ...
        for i in range(nums_restart):
            if len(final_cands) < nums_samples:
                for center in centers:
                    # center = self.X[ np.random.randint( len(self.X) ) ]
                    cands  = sobol.draw(10000).to(dtype=torch.float64).cpu().detach().numpy()
                    ratio  = 1
                    L      = 0.0001
                    Blimit = np.max(ub - lb)
        ...
        # nums_remain = nums_samples - len(final_cands)
        # final_cands.extend(self.propose_rand_samples( nums_remain, lb, ub ))
        # final_cands = np.array( final_cands )
        # assert len(final_cands) == nums_samples
        if len(final_cands) > 0:
            return np.array( final_cands )
        return self.propose_rand_samples( nums_samples, lb, ub )
    
    def propose_samples_transformer(self, algo, nums_samples = 10, path = None, lb = None, ub = None, samples = None, source_samples = None, model_path=None):
        ...
        if len(path) == 0:
            return self.propose_rand_samples(nums_samples, lb, ub)
        X    = self.propose_rand_samples_sobol(nums_rand_samples, path, lb, ub, source_samples)
        # assert len(X) == nums_rand_samples
        if len(self.X) == 0:
            return X[np.array([np.random.randint(len(X))])]
        return algo.ask_by_mcts(X, model_path).numpy()
            
    def propose_samples_bo( self, nums_samples = 10, path = None, lb = None, ub = None, samples = None, source_samples = None, local = False):
        ...
        if samples and len(samples) > 0:
            if local==False:
                self.train_gpr( samples, lb, ub ) # learn in unit cube 
            elif len(self.X) > 0:
                self.train_gpr_local(samples, lb, ub)
        ...
        X_ei = self.expected_improvement(X, lb, ub, xi=0.001, use_ei = True)
        # row, col = X.shape
    ...
    def split_data(self):
        ...
        for idx in range(0, len(plabel)):
            if plabel[idx] == 0:
                ...
                if self.stage == 0:
                    good_source_id.append(self.node.source_id[idx])
                train_good_samples.append( self.X[idx] )

            else:
                bad_samples.append( self.samples[idx] )
                if self.stage == 0:
                    bad_source_id.append(self.node.source_id[idx])
        ...
        if self.stage == 0:
            return  good_samples, bad_samples, good_source_id, bad_source_id
        return  good_samples, bad_samples, None, None
