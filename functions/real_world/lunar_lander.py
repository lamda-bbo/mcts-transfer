...
class LunarLanderProblem(BaseTestProblem):
    dim: int = 12
    num_objectives = 1
    lb = np.zeros(dim) # for algorithm
    ub = np.ones(dim) # for algorithm
    ...
    def __call__(self, x):
        ...
        return self.evaluate_true(x).item()*-1

# def lunar(x):
#     x = torch.tensor(x.reshape(1,-1))
#     prob = LunarLanderProblem()
#     return prob.evaluate_true(x).item()*-1

# def get_dim():
#     return LunarLanderProblem().dim

if __name__ == '__main__':
    # print(demo_heuristic_lander(LunarLander(), render=False))
    prob = LunarLanderProblem()
    dim = prob.dim
    # x = torch.tensor(np.random.uniform(np.zeros(dim), np.ones(dim)))
    x = torch.tensor(np.random.uniform(np.zeros(dim), np.ones(dim)).reshape(1,-1))
    # print(lunar(x))
    print(prob(x))
    # print(prob.evaluate_true(x).item())